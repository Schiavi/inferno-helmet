...
const generateTagsAsString = (type, tags, encode) =>
    tags.reduce((str, tag) => {
        ...
        const isVoid = VOID_TAGS.indexOf(type) !== -1;

        return `${str}<${type} ${HELMET_ATTRIBUTE}="true" ${attributeHtml}${
            isVoid ? `>` : `>${tagContent}</${type}>`
        }`;
    }, "");

const convertElementAttributestoInfernoProps = (attributes, initProps = {}) => {
...
const convertVNodeToHtmlAttributes = ({className, props}) => {
    const initialAttributes = className
        ? {[HTML_TAG_MAP.className]: className}
        : {};
...
const generateTitleAsInfernoComponent = (type, title, attributes) => {
    ...
    const props = convertElementAttributestoInfernoProps(attributes, initProps);

    return [
        createVNode(
            getFlagsForElementVnode(TAG_NAMES.TITLE),
            TAG_NAMES.TITLE,
...
const generateTagsAsInfernoComponent = (type, tags) =>
    tags.map((tag, i) => {
        const mappedTag = {
            [HELMET_ATTRIBUTE]: "true"
        };
        ...
        return createVNode(
            getFlagsForElementVnode(type),
            type,
            mappedTag.className || null,
            mappedTag.children || undefined,
            0, // UnknownChildren
            mappedTag,
            i,
            mappedTag.ref || null
...
const getMethodsForTag = (type, tags, encode) => {
    switch (type) {
        case TAG_NAMES.TITLE:
            return {
                toComponent: () =>
                    generateTitleAsInfernoComponent(
                        type,
                        tags.title,
        ...
        case ATTRIBUTE_NAMES.HTML:
            return {
                toComponent: () => convertElementAttributestoInfernoProps(tags),
...
export {convertVNodeToHtmlAttributes};
